...
<div class="main_box">
	...
	<form action="#">
		<div class="search_box clearfix">
			<input type="text" class="f_left" value="<?php echo ($_GET['keywords']); ?>" name="keywords" placeholder="输入轮播图名称">
			<input type="submit" value="查询" class="btn blue_btn">
			<button type="button" link="<?php echo U('Banner/banner_edit');?>" onclick="link(this)" class="btn blue_btn">添 加</button>
		</div>
	</form>
	<div class="cont_box">
		<table border="0" cellspacing="0" cellpadding="0" class="table" id="table_box">
			...
			<?php if($data): if(is_array($data)): foreach($data as $key=>$v): ?><tr>
						...
						<td>
							<?php if($v['status'] == 0): ?><a class="table_btn table_grey down_shelf" onclick="setfield(this,'banner','<?php echo ($v["id"]); ?>')" >
									<i class="fa fa-eye-slash"></i>
									<span field="status" value="1">隐藏</span>
								</a>
							<?php else: ?>
							    <a class="table_btn table_warning up_shelf" onclick="setfield(this,'banner','<?php echo ($v["id"]); ?>')">
									<i class="fa fa-eye"></i>
						...
						<td>
						    <a href="<?php echo U('Banner/banner_edit',array('id'=>$v['id']));?>" class="table_btn table_edit edit_btn">
								<i class="fa fa-edit"></i>
								<span>编辑</span>
							</a>
							<a link="<?php echo U('Banner/banner_delete');?>" key="<?php echo ($v["id"]); ?>" class="table_btn table_del del_btn">
