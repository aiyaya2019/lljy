...
<div class="main_box">
    <h2><span></span>视频添加</h2>
    <div class="cont_box">
        <form action="<?php echo U('Video/video_edit',array('id'=>$data['id']));?>" method="post" id="form">
            <ul class="addpro_box adduser_box">
                <li>
                    <label>视频名称：</label>
                    <input type="text" placeholder="请输入视频名称" name="title" value="<?php echo ($data["title"]); ?>" />
                </li>
                <li>
                    <label>所属分类：</label>
                    <select name="cate_id">
                        <option value="0">===请选择分类===</option>
                        <?php if(is_array($cate)): foreach($cate as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php if($v['id'] == $data['cate_id']): ?>selected<?php endif; ?> ><?php echo ($v["html"]); echo ($v["name"]); ?></option><?php endforeach; endif; ?>
                    </select>
                </li>
                <li>
                    <label>上映时间：</label>
                    <?php if($data['show_time'] == 0): ?><input type="text" placeholder="请选择上映时间" name="show_time" id="show_time" value="" />
                    <?php else: ?>
                        <input type="text" placeholder="请选择上映时间" name="show_time" id="show_time" value="<?php echo (date('Y-m-d',$data["show_time"])); ?>" /><?php endif; ?>
                </li>


                <li>
                    <label>费用：</label>
                    <?php if($data['is_pay'] != null): if($data['is_pay'] == 1): ?><div class="radio_box is_pay">
                                <i class="fa fa-1x fa-check-circle"></i>
                                <input type="radio" name="is_pay" value="1" class="input_radio" checked="checked"><span>付费</span>
                            </div>
                            <div class="radio_box is_pay">
                                <i class="fa fa-1x fa-circle-thin"></i>
                                <input type="radio" name="is_pay" value="0" class="input_radio"><span>免费</span>
                            </div>
                        <?php else: ?>
                            <div class="radio_box is_pay">
                                <i class="fa fa-1x fa-circle-thin"></i>
                                <input type="radio" name="is_pay" value="1" class="input_radio"><span>付费</span>
                            </div>
                            <div class="radio_box is_pay">
                                <i class="fa fa-1x fa-check-circle"></i>
                                <input type="radio" name="is_pay" value="0" class="input_radio" checked="checked"><span>免费</span>
                            </div><?php endif; ?>
                ...
                <li class="ispay hide">
                    <label>价格：</label>
                    <input type="text" placeholder="请输入价格" name="price" value="<?php echo ($data["price"]); ?>" />
                </li>
                <!-- <li class="ispay hide">
                    <label>付费有效期/月：</label>
                    <input type="text" placeholder="请输入用有效" name="term" value="<?php echo ($data["term"]); ?>" />
                </li> -->
                ...
                <li>
                    ...
                    <div id="upload_pic">
                        ...
                        <?php else: ?>
                            <img src="<?php echo ($data["pic"]); ?>" class="pic" alt="封面图"><?php endif; ?>
                    </div>
                    <input type="hidden" name="pic" value="<?php echo ($data["pic"]); ?>"/>
                </li>
                <li>
                    <label>视频：</label>
                    <div id="upload_video"><i class="fa fa-plus-square-o fa-3x pic"></i></div>
                    <input type="text" name="video" value="<?php echo ($data["video"]); ?>" style="margin:30px 10px;"/>
                </li>
                <li>
                    <label>简介：</label>
                    <textarea name="desp" id="desp"><?php echo ($data["desp"]); ?></textarea>
                </li>
                <li>
                    <label>内容：</label>
                ...
                <li>
                    <label>是否推荐：</label>
                    <?php if($data['is_red'] != null): if($data['is_red'] == 1): ?><div class="radio_box is_red">
                                <i class="fa fa-1x fa-check-circle"></i>
                                <input type="radio" name="is_red" value="1" class="input_radio" checked="checked"><span>是</span>
                            </div>
                            <div class="radio_box is_red">
                                <i class="fa fa-1x fa-circle-thin"></i>
                                <input type="radio" name="is_red" value="0" class="input_radio"><span>否</span>
                            </div>
                        <?php else: ?>
                            <div class="radio_box is_red">
                                <i class="fa fa-1x fa-circle-thin"></i>
                                <input type="radio" name="is_red" value="1" class="input_radio"><span>是</span>
                            </div>
                            <div class="radio_box is_red">
                                <i class="fa fa-1x fa-check-circle"></i>
                                <input type="radio" name="is_red" value="0" class="input_radio" checked="checked"><span>否</span>
                            </div><?php endif; ?>
...
<!-- 日期 -->
<script src="/Public/plug/laydate/laydate.js?v=1"></script>
<script>
    //执行一个laydate实例
    laydate.render({
        elem: '#show_time' //指定元素
...
<script type="text/javascript">
    $('.is_pay').click(function(){
        var cheched = $(this).children('input').val();
        if (cheched == '1') {
