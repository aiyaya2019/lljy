...
<html>
	...
	<body>
<div class="main_box">
	<h2><span></span><?php if($_GET['pid']): ?>添加【<?php echo ($_GET['cate_name']); ?>】的子分类<?php else: ?>商品分类添加<?php endif; ?></h2>
	<div class="cont_box">
		<form action="<?php echo U('Product/number_editHandle',array('id'=>$data['id']));?>" method="post" id="form">
			<ul class="addpro_box adduser_box">
				<li>
					<label>可领取次数：</label>
					<input type="text" placeholder="请输入可领取次数" name="num" value="<?php echo ($data["num"]); ?>" />
				</li>
                <li>
                    <label>图标：</label>
                    <div id="upload_pic">
			...
			<div class="probtn_box clearfix">
				<input type="hidden" class="btn" name="pid" value="<?php echo ((isset($_GET['pid']) && ($_GET['pid'] !== ""))?($_GET['pid']):0); ?>"/>
...
<script type="text/javascript">
    var uploader_avatar = new plupload.Uploader({   //创建实例的构造方法
        ...
        url: "<?php echo U('Common/uploadsImg',array('path'=>number));?>",   //远程上传地址
        filters: {
